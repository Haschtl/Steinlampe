...
3. Add the *Quarzlampe* integration via *Settings → Devices & Services* and choose transport:
   - **BLE**: enter the lamp’s BLE MAC (service UUID `d94d86d7-1eaf-47a4-9d1e-7a90bf34e66b`).
   - **BT Serial**: select `bt_serial` and point to an existing serial port (e.g. `/dev/rfcomm0`) that is already paired/bound to the lamp’s Classic BT SPP. Default baud 115200.
